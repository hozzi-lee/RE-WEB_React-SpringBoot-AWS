...
public class TodoController {
	...
	public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto) {
		try {
			...
			//(3) 임시 사용자 아이디를 설정.
			//해당 부분은 4장 인증/인가에서 수정할 예정
			//현재 소스는 인증/인가가 없으므로 한 사용자("temporary-user")만 사용 가능한 어플리케이션임.
			entity.setUserId(temporaryUserId);
			
			//(4) 서비스는 이용해 Todo 엔티티 생성
			List<TodoEntity> entities = service.create(entity);
			
			//(5) 자바 스트림을 이용해 리턴된 엔티티 리스트를 TodoDTO 리스트로 변환
			List<TodoDTO> dtos = entities.stream().map(TodoDTO::new).collect(Collectors.toList());
			
			//(6) 뱐환된 TodoDTO 리스트를 이용해 ResponseDTO를 초기화
	...
	public ResponseEntity<?> retrieveTodoList() {
		...
		//(1) 서비스 메서드의 retrieve() 메서드를 사용해 Todo 리스트를 갖고옴.
	...
	public ResponseEntity<?> updateTodo(@RequestBody TodoDTO dto) {
		...
		//(2) id를 temporaryUserId로 초기화. 4장 인증/인가에서 수정 예정.
		entity.setUserId(temporaryUserId);
		
		//(3) 서비스를 이용해 entity를 업데이터.
	...
	public ResponseEntity<?> deleteTodo(@RequestBody TodoDTO dto) {
		try {
			...
			//(5) 뱐환된 TodoDTO 리스트를 이용해 ResponseDTO를 초기화.
			ResponseDTO<TodoDTO> response = ResponseDTO.<TodoDTO>builder().data(dtos).build();
